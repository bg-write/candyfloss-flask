...
**Candyfloss** is a digital daily newspaper curating the best music news and longform writing (and occasional sports link). Now you don't need Twitter to keep up on the music world. Styling inspired by the print covers of the London Review of Books.

Candyfloss displays the 50 most recent links pooling from several outlets and is set to refresh at the top of every hour. All you have to do is open up Candyfloss, click on whatever links you want, and enjoy!
...
A fellow music journalist or music fan who wants to discover some of the best music outlets today without having to be on social media.

## Why Python?

Python is one of my favorite languages; I love its balance of power and simplicity. I mostly work in JavaScript and wanted more hands-on work with Python, so I decided to use Python to make something I would actually use myself. I also wanted to take advantage of and learn Beautiful Soup, a Python library for parsing structured data.

I deployed Candyfloss as a Flask app via PythonAnywhere, which allows me to host the app on a separate domain and keep track of basic analytics with an affordable paid account. (In my experience, PythonAnywhere is easier to work with than Heroku and AWS, though it's not as powerful and flexible and only works with Python.)
...
In your IDE of choice, in an open terminal window where (candyfloss-venv) is running, enter and run `flask --debug run` and open the development server URL provided in the terminal output. (NOTE: you may have to delete your current terminal window and open a new window to make sure (candyfloss-venv) is running.)
...
1) **"Get" the soup**: make my GET request
2) **"Cook" the soup**: pinpoint the repeating element holding the content I wish to pull from
3) **"Deliver" the soup**: use a `for` loop to append the info I need into my empty lists, which I then combine into a new dict, which looks like:
